...
export default function NewCategory({ refreshCategories, closeDialog }: NewCategoryProps) {
  const [formCategory, setFormCategory] = useState({
    name: "",
  });

  const handleInputCategory = (e: { target: { id: string; value: string } }) => {
  ...
  const submitFormCategory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        refreshCategories(); // Recarrega as categorias
